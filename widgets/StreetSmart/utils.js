define(["esri/SpatialReference","esri/geometry/Point"],function(SpatialReference,Point){"use strict";return{proj4:null,DEG_TO_RAD:Math.PI/180,RAD_TO_DEG:180/Math.PI,toRadians:function toRadians(deg){return deg*this.DEG_TO_RAD},toDegrees:function toDegrees(rad){return rad*this.RAD_TO_DEG},setProj4:function setProj4(proj4){this.proj4=proj4},/**
         ...
         */transformProj4js:function transformProj4js(sourceGeom,targetSrs){//NB: Only works with points.
//No transformation needed if source SRS == target SRS
if(sourceGeom.spatialReference.wkid===targetSrs){return sourceGeom}var sourceEpsg="EPSG:"+sourceGeom.spatialReference.wkid;var destEpsg="EPSG:"+targetSrs;if(sourceEpsg==="EPSG:102100"){sourceEpsg="EPSG:3857"}if(destEpsg==="EPSG:102100"){destEpsg="EPSG:3857"}var source=this.proj4(sourceEpsg);var dest=this.proj4(destEpsg);var p=this.proj4(source,dest).forward([sourceGeom.x,sourceGeom.y]);return new Point(p[0],p[1],new SpatialReference({wkid:parseInt(targetSrs)}))}}});